...
function buscarModoPago(modoPago, modo)
{
    let valor = ''
...
function reportarPagomovil(datosCliente, transaccion1) {
    transaccion = transaccion1
    let principal = document.getElementById('principal')
    principal.innerHTML = ''
    let textCard = buscarModoPago(modoPago, transaccion['modopago'])
    let textButton1 = 'Continuar'
    let textButton2 = 'Volver'

    let card1 = document.createElement('div')
    card1.classList.add('card', 'my-3')
        ...
        if(index != 3){
            if(index == 2){
              ...
              if(index !== 1 || transaccion.modopago !== 'transferencia'){
                ...
                colRowB1.classList.add('col-lg-6')
                colRowB2.classList.add('col-lg-6')
    ...
    colRow1.classList.add('col-lg-6')
    colRow2.classList.add('col-lg-6')
...
function reportarZelle(datosCliente, transaccion1) {
  transaccion = transaccion1
  let principal = document.getElementById('principal')
  principal.innerHTML = ''
  let textCard = buscarModoPago(modoPago, transaccion['modopago'])
  ...
  divG.classList.add('form-group')
  let label = document.createElement('label')
  label.for = "codigoZelle"
  let codigoZelle = document.createElement('input')
  codigoZelle.classList.add('form-control')
  codigoZelle.id = "codigoZelle"
  codigoZelle.placeholder="Ingrese el código"
  codigoZelle.value = ''

  divG.appendChild(label)
  divG.appendChild(codigoZelle)
  bodyC.appendChild(divG)
