            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active"><a href="/listComercios/1">Comercios</a></li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                        ...
                        <div class="card-body">
                            <table class="table table-hover table-bordered">
                                ...
                                <tbody wire:loading.class="text-muted">
                                    ...
                                    <tr>
                                        <th scope="row">{{ $clientes->firstItem() + $index }}</th>
                                        <td>
                                            <img src="{{ $cliente->avatar_url }}" style="width: 50px;" class="img img-circle mr-1" alt="">
                                            {{ $cliente->user->name }}
                                        </td>
                                        <td>{{ $cliente->user->email }}</td>
                                        <td>{{ $cliente->datosbasicos->cellphonecode }}-{{ $cliente->datosbasicos->cellphone }}</td>
                                        <td>{{ $cliente->user->created_at->toFormattedDate() ?? 'N/A' }}</td>
                                        <td>

                                            <a href="" wire:click.prevent="edit({{ $cliente }})">
                                                <i class="fa fa-edit mr-2"></i>
                                            </a>

                                            <a href="" wire:click.prevent="confirmUserRemoval({{ $cliente->id }})">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" wire:model.defer="state.names" class="form-control @error('names') is-invalid @enderror" id="names" aria-describedby="namesHelp" placeholder="Nombres">
                            @error('names')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" wire:model.defer="state.surnames" class="form-control @error('surnames') is-invalid @enderror" id="surnames" aria-describedby="namesHelp" placeholder="Apellidos">
                            @error('surnames')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
    ...
    <div class="modal fade" id="confirmationModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h4>Esta seguro de querer eliminar este cliente?</h4>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="fa fa-times mr-1"></i> Cancelar</button>
                    <button type="button" wire:click.prevent="deleteUser" class="btn btn-danger"><i class="fa fa-trash mr-1"></i>Eliminar Cliente</button>
                </div>
            </div>
        </div>
    </div>
